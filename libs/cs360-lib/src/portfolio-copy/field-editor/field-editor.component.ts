...
import { includes, keys, forEach, isEmpty, uniq, get } from 'lodash';
...
import { getFieldId, getUrlValidator } from '../portfolio-widget-utils';
...
export class FieldEditorComponent implements OnInit, AfterViewInit, OnChanges {
  ...
  constructor(public fqs: FilterQueryService,@Inject("envService") public _env: EnvironmentService,
  public _fb: FormBuilder, public portfolioWidgetService: PortfolioWidgetService, public userService: UserService,protected cdRef: ChangeDetectorRef,public i18nService?: NzI18nService) {
    this.dateFormat = this.getUserConfigByHostType() && this.getUserConfigByHostType().locale ? this.getUserConfigByHostType().locale.dateFormat : 'M/d/yyyy';
  ...
  getFormattedValue(value: any, field) {
    ...
    switch(field.dataType) {
      ...
      case DataTypes.DATE:
        ...
        const GSObject = this._env.getConfig();
  ...
  getSelectedValue(value: any, field): InlineEditData {
    ...
    switch(field.dataType) {
      ...
      case DataTypes.STRING:
        // Assume selected time is in given timezone (timeZoneKey), now convert from timeZoneKey to UTC
        selectedValue.k = purifyDomForString(value);
